...
|---|---|
| **Inline `/workboard`** | Cross-initiative dashboard is a procedure + format refs, not a `ba-workboard` sub-skill |
| **BA actions** | Personal tasks live in `_workstream/ba-actions.json` (`/todo`, debrief sync, `/wrap`) |
| **Unified requirements register** | One register; Mode 4 Kickoff HLR review before tech kickoff |
| **AskQuestion** | Used more at forks, re-entry, and runthroughs; model-tier nudges removed (Auto-balance) |
| **Upgrade script** | `tools/upgrade-ba-assistant.py` upgrades older installs without wiping `ba-profile.mdc` |
| **Calendar samples** | Windows + macOS samples; calendar optional |

---

## What changed recently (Waves 1–7 foundations)

If you've used the BA Assistant before, here's what landed in earlier waves:

| Change | What it means for you |
|---|---|
| **Active workstreams** replace sequential phases | Workstreams can run in parallel for different scopes |
| **Scope navigator** | Scope to initiative / feature / cohort / slice |
| **MoSCoW per scope** | Must/Should/Could/Won't per scope |
| **Standards index** | Artefact standards in `references/` |
| **Scope label naming** | Stakeholder-facing output uses real business names, never abstract "Feature A" |
| **New commands** | `/metrics`, `/reanchor`, `/validate-state`, `/wrap`, `/workboard`, `/todo`, `/handover` |
...
| `user-story-format.md` | Stories, spikes, bugs, enablers, DoR checklist | Delivery Definition |
...
The critique is surfaced transparently  -  not hidden. You'll see lines like "*Self-critique: I'm assuming [Team Member] is the right decision-maker for ADR-01  -  should we verify this?*"
