...
*Updated: May 2026 — covers Wave 1, Wave 2, Wave 3, and Wave 4 changes.*
...
| **20 active skills (Wave 3)** | Down from 21 standalone skills — 6 were merged into related skills (no functionality lost). |
...
|---|---|
| `/next` | Top 3 next actions based on context, missing inputs, and critical path |
| `/status` | Concise live status — modes per scope, top blockers, decisions made this week, MoSCoW coverage |
...
**Note on Cursor slash menus:** Cursor doesn't currently offer Claude-style button prompts. The assistant uses `AskQuestion` with clickable options for every decision point, which is the closest equivalent. See `slash-commands-ux.md` (Wave 4 exploration) for documented patterns.
...
**Backwards compatible:** if your initiative is single-scope and linear (one feature, one cohort), workstreams run in the old sequential order. You won't notice the change. Dual-track teams (e.g. [Example Project]) benefit from running Discovery on Feature B while Delivery is active on Feature A.
...
|---|---|
| `Anti_Pattern_Detector` | Premature solutioning, skipped slicing, missing stakeholders, missing MoSCoW, workstream-aware combinations (Solution active without Discovery complete, Delivery across 3+ scopes without resource declaration, late discovery, workstream thrashing) |
...
**The warn-and-flag gate (at Delivery Definition):**
...
The critique is surfaced transparently — not hidden. You'll see lines like "*Self-critique: I'm assuming [PM Name] is the right decision-maker for ADR-01 — should we verify this?*"

### Cross-initiative learnings

A file called `learnings.md` persists across initiatives. At every initiative close (or significant mid-initiative retro), the Retrospective skill writes patterns, watchlist items, and skill refinements to it. At the start of every new initiative, the Intake Reviewer reads it.

This means recurring failure modes get caught earlier. If three previous initiatives all hit the same pattern, the fourth one will be probing for it from intake.
...
2. **Use the form intake** — Wave 4 batches workspace setup questions; fill what you know, leave blanks for gaps.
3. **Answer honestly** — it's fine to say "I don't know" or "I need to ask compliance." The Assistant logs unknowns and suggests next steps.
4. **Use commands** — `/next` when stuck, `/status` for a live snapshot, `/report` for a full document, `/snapshot` for the tracker.
5. **Choose output modes** — specify `quick summary`, `confluence-ready`, `thinking`, or `workshop plan` when generating a summary or report.
6. **Switch scope** — on the canvas, drill into a feature or cohort to see only its modes, tracker, MoSCoW.
7. **Push back** — if the Assistant flags a risk or suggests an action, you can accept, question, or defer. If you proceed at risk, the decision is logged.
8. **Iterate** — modes loop back. You can loop into discovery after slicing, update requirements during solution shaping, raise change tickets during delivery. The Assistant adapts.
9. **Drop meeting transcripts** — after meetings, paste the transcript; `Meeting_Debrief` extracts decisions, actions, OQs, and updates relevant skills/tracker.
10. **Ask for visuals** — "show me the dependency diagram", "make me a one-pager for this", "draft an email to [PM Name]." The Assistant produces them.
...
The BA Assistant is a thinking partner, not a note-taker. It asks the right questions, structures your outputs, keeps you honest about what's known and unknown, supports dual-track delivery, and produces the artefacts that would otherwise eat your time — visuals, draft communications, ticket drafts, retrospectives, and post-launch evaluations.
