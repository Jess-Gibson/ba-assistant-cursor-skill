...
| Detailed merchant comms before approach confirmed | Placeholder comms  -  draft only after key vendor/stakeholder confirmation |
...
**Why this exists:** Sample-Compliance-Initiative retro (23 Jun 2026) showed that the standard orchestrator created overhead for a 6-day initiative  -  formal canvases and detailed comms were created before the approach was confirmed, resulting in wasted work. The fast-track mode front-loads engineering consultation, defers artifact ceremony, and focuses on the 3-4 outputs that drive decisions.
...
| `references/jira-ticket-format.md` | Cross-cutting Jira write rules (positioning file) | jira-templates, any project-specific Jira skill |
...
3.5. **Mandatory Downloads check (every resume, not just `/workboard`):** run `cmd /c dir "[Downloads folder - set BA_DOWNLOADS_PATH]" /a-d /o-d`  -  never `Get-ChildItem` or PowerShell/.NET directory enumeration, both silently return empty for this folder. Check the **last 7 days** of files (any file type  -  `.docx`, `.csv`, transcripts, exports), cross-reference filenames/dates against what's already been debriefed in SESSION-CONTEXT.md and initiative-tracker.md, and flag anything unprocessed in the re-entry card below. Do not rely on `CURSOR_NEW_TRANSCRIPTS` alone  -  that env var can miss files the hook didn't catch; the manual `dir` check is the source of truth. **Exception  -  `/debrief` command:** skip step 3.5 entirely; the meeting-debrief skill uses a **3-day** scoped list only (`list-downloads-recent.ps1 -Days 3`), or skips Downloads when the user attached transcript path(s).
4. **Present the re-entry card (mandatory  -  replaces the old resume summary block):**
  ...
  For "New transcripts": use the mandatory Downloads check (step 3.5) as the source of truth; `CURSOR_NEW_TRANSCRIPTS` env var (set by session-init hook) is a supplementary signal only, not a substitute.
...
- Capture PM name (e.g. "[Team Member]") in `status-data.json → initiative.pmApproval.pm`.
...
3. **Cohort or Slice scope**  -  finer subdivision of a feature. Some initiatives use cohorts (e.g. Data Collection Uplift Project 002 uses merchant cohorts); others slice by region, customer tier, technical layer, etc.
...
| Quick T2P          | Delivery (M5) active, Playback (M6) starting                            |
| Full T2P draft     | Discovery (M2) active; Solution (M4) active for spike S-04              |
| Existing OIP <12mo | Slicing (M3) complete, Solution (M4) complete, Delivery (M5) active     |
| Existing OIP 12+mo | Discovery (M2) active, Slicing (M3) not started                         |
| PROJ-4287 spike     | Solution (M4) active                                                    |
...
| "Feature B blocked on CD-5"        | "Data Minimisation blocked on CD-5"     |
| "Cohort A engineering active"      | "Stale Drafts engineering active"       |
| "Cohort B paused until BL-LEGAL-1" | "Churned >7yr paused until BL-LEGAL-1"  |
...
**Prefer `AskQuestion` when it keeps work moving, per `agent-behavior.mdc`.** Use it for forks, re-entry cards, runthroughs, closure ceremonies, and non-obvious next steps. The four authoring rules above govern HOW to ask. Skip only for verbatim artefacts, a single obvious next step, trivial confirmations, or "anything else?" filler.
