...
1. **Mail check (light).** If Outlook is connected through MCP, do a light Inbox and Sent check since today (or the last working day after a morning gap): open threads needing a reply, and Sent items that create a follow-up. Recommend a next step or draft a reply for each. If Outlook is not connected, say "Mail: unable to check" and continue. A mail failure never stops the rest of end of day.
