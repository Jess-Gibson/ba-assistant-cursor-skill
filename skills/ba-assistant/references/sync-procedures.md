...
trigger fires or `/validate-state` runs. `/wrap` does not load this file: it follows `commands/wrap.md`. The 20-turn breakpoint
...
| **Sync gate** | See triggers below | Run sync check, present sync card if drift detected. `/validate-state` and `/wrap` both write what this chat captured. |

## Commands (simplified)

| Command | Purpose | Mutates? |
|---------|---------|----------|
| `/validate-state` | `/validate-state` checks this chat against the current initiative. Write what this chat captured into the right files: `SESSION-CONTEXT.md`, the tracker, and `status-data.json` where those files already exist, and upsert only the BA actions that came from this chat into `~/.cursor/_workstream/ba-actions.json`. Do not refresh the workboard. Do not walk every open action. Do not invent files. If something has no obvious home, say so and ask. | Yes (this chat only) |
| `/wrap` | Chat-only checkpoint, defined in `commands/wrap.md`. Captures this chat into `SESSION-CONTEXT.md` and the tracker. No workboard refresh. | Yes (this chat only) |

**`/sync-check` is retired.** Use `/validate-state` to write this chat into the initiative files, or `/wrap` to close out the chat. If the user types `/sync-check`, treat it as `/validate-state`.
...
2. **Before switching initiatives** -- when the user says "[other initiative] resume" after working on Sample Initiative, run sync on the OUTGOING initiative before loading the new one.
3. **Before any Confluence publish** -- check that local files are current before pushing stale data to Confluence.
4. **On `/validate-state` command** -- user-initiated. `/validate-state` checks this chat against the current initiative. Write what this chat captured into the right files: `SESSION-CONTEXT.md`, the tracker, and `status-data.json` where those files already exist, and upsert only the BA actions that came from this chat into `~/.cursor/_workstream/ba-actions.json`. Do not refresh the workboard. Do not walk every open action. Do not invent files. If something has no obvious home, say so and ask.
5. **On `/wrap` command** -- chat-only checkpoint per `commands/wrap.md`. Not end of day.
6. **At 20 turns in a BA thread** -- regardless of topic drift. Run a lightweight mid-session sync: check for unpromoted items in SESSION-CONTEXT, promote if >3 items, suggest a checkpoint. Present as a brief inline note, not a full sync card. If the user defers, don't ask again until 40 turns.
...
When a sync gate fires and detects unpromoted items (Step 1), the agent SHOULD auto-promote them.
...
## `/wrap` and end of day (not in this file)

- **`/wrap`** is defined in `commands/wrap.md` (installed at `~/.cursor/commands/wrap.md`). It is a chat-only checkpoint: capture this chat into `SESSION-CONTEXT.md` and the tracker. No workboard refresh, no Downloads scan, no calendar reconciliation, no walk of every action, no handoff block.
- **End of day** is `references/eod-closeout-procedure.md`, run via `/workboard end-of-day` or the workboard's End of Day button. It owns the action runthrough, meeting recall, and workboard refresh.
