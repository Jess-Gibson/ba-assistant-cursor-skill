...
2. **Before switching initiatives** -- when the user switches between initiatives, run sync on the OUTGOING initiative before loading the new one.
...
6. **On `/wrap` command** -- session closeout: run **action runthrough** (see below), then validate-state, then fix all drift + refresh workboard + checkpoint.
...
Status-data drift: PROJ-001 is "Done" in Jira but "In Progress" in status-data
...
Before running the state validation, walk through **every open action** in the actions list one at a time. For each action:

1. Present the action description and current recorded status via **AskQuestion** with status options (Done, In progress, Blocked, No update, etc.)
2. Capture any verbal context the user provides (new facts, decisions, status changes, follow-up tasks)
3. Update the actions list with the new status
4. If the user surfaces new actions or tasks during the runthrough, add them

This pattern works because:
- It forces a structured review of every open item at end of day
- It surfaces updates that wouldn't come up organically
- It captures verbal context and turns it into formal state updates
- It naturally identifies new actions and follow-ups

After the runthrough, present a summary table of all actions with updated statuses, then proceed to state validation.
...
4. **workboard.json** -- derived from all initiative trackers + Jira + calendar
5. **Confluence pages** -- derived from local files, published on command
6. **Canvas / HTML snapshots** -- derived from status-data.json, regenerated on demand
