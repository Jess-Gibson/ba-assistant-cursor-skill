...
description: Pairs with the user to gather, question, cross-validate, and debug data before a BA decision (confidence score, priority, risk rating, solution comparison, baseline, or post-launch actual) gets locked in on judgement alone. Invoked by other BA Assistant skills via hooks. Encodes a disciplined data-investigation methodology, not a generic analytics workflow.
---
...
The method below is not generic analytics practice — it's a disciplined investigation method: state the question precisely, rank every candidate source with a status and a reason, do a row-level sanity pass before trusting any aggregate, annotate SQL so it's auditable, cross-validate against a second source whenever one exists, and treat any disagreement between sources as the finding rather than noise to average away. Assumptions that can't yet be checked don't block — they get logged, tagged, and routed back to the calling skill.
...
When a decision or calculation is anchored to a number a stakeholder gave verbally or approximately (e.g. "roughly 70,000 customers", "about a third of them") rather than a number pulled directly from a validated source, treat that number with the same rigor as a warehouse query result before building on it:

- **Confirm the population/segment it refers to** — ask or infer precisely which filter set produces that number (e.g. does "70,000" mean a specific enabled cohort, all active accounts, or something else?), and check it against an actual query rather than assuming the most obvious reading.
...
Skipping this step is how an anchor number silently becomes the wrong base population for a whole downstream calculation. *(Added from a mid-initiative retrospective — a case where a decision was locked in against an approximate stakeholder anchor without cross-checking it against an available explicit source.)*

### 1. State the question precisely

Before touching any data, restate: the decision this data will inform, the metric, the grain (daily / per-invoice / per-customer / per-cohort), the filters, and the time window. If the calling skill's hook didn't supply enough of this, ask before proceeding — a vague question produces a query that answers the wrong thing.

### 2. Identify and rank candidate sources

List every plausible source — warehouse tables, Jira, Confluence, log/monitoring tools, product telemetry, support tickets. Tag each:
...
Decide and say out loud: rates/shares vs absolute counts (absolute counts are noisy under seasonal/volume swings — prefer shares unless the decision genuinely needs a raw count), and the baseline period (name it, and name what's excluded and why — e.g. "exclude the peak-season week, it distorts volume").
...
2. Trace it to a mechanism — different grain, different event semantics (e.g. "UI send intent" vs "confirmed delivery"), different join, different completeness lag. Don't stop at "the numbers don't match."
...
When a question can't be answered without input from a data-owning stakeholder (engineering, analytics, another team), log it:
...
|---|---|---|
| Warehouse / SQL | Data warehouse MCP (e.g. Snowflake) | `pm-data-analyst` → `references/warehouse-and-sql.md` (reused here for query execution mechanics, not duplicated) |
| Jira ticket volumes/history | Jira MCP | `ba-jira-sync` |
| Confluence metrics pages | Confluence MCP | this skill, direct read |
| Production/incident data | Log/monitoring tool (e.g. Sumo Logic, Datadog) | production-support / troubleshooting skill, if installed |
...
`pm-data-analyst` is a separate, generic skill built for standalone PM analytics — ad hoc CSV/telemetry analysis requested directly by the user outside a BA Assistant decision point. It remains available and untouched for that use.
...
- **Never report a discrepancy without attempting to explain the mechanism.** "The numbers don't match" is not a finding; "one source counts UI send intent, the other counts confirmed delivery — that's the gap" is a finding.
