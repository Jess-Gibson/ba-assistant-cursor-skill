...
- `references/requirements-register-unified-template.md`  -  v2 register layout, ID scheme, **scope writing rules (§2a): short In/Out bullets; detail in child rows**
...
6. **Requirement capture and interrogation** – Requirements enter `register.md` immediately on capture, at `status: proposed`, from any intake (transcript, Confluence, verbal, Jira, workshop)  -  as a one-line intake row, not the full write-up (see `requirement-format.md` §3a). Interrogation is not a gate on entry; it's what advances a requirement past `proposed`. For each candidate requirement:
   ...
   - Each requirement carries: identifier, provisional statement (from Interrogator), evidence weight, source, dependencies, acceptance criteria (if known), and who signs it off to move it to `confirmed`
...
8. **Evidence gathering** – Suggest data queries or analysis to validate assumptions or quantify pain points. Provide concrete query templates where the data source is known (e.g., warehouse query for volume, logs query for error rate).
...
12. **Requirements lifecycle states** – Every requirement in the register carries a single `status` field. This skill does not maintain its own lifecycle vocabulary  -  **the canonical states, transitions, and the `blockedOn` sub-field live in `requirement-format.md` §3b/§3c; that file is authoritative.** State transitions are recorded with date, owner, and reason (lightweight change history  -  not full requirements management tooling).

    **Quick reference** (see `requirement-format.md` for the full picture):
    ...
    | `interrogated` | Requirements Interrogator has produced a provisional statement; may be `blockedOn` a spike/open-question/design/compliance/decision | Discovery mode interrogation complete |
    | `confirmed` | PM/PO has explicitly signed off the provisional statement | User has explicitly validated the provisional statement, and `blockedOn` is `none` |
    ...
    | `descoped` | Real requirement, deliberately not being built this cycle (distinct from `rejected`) | Decision logged |
    | `superseded` | Replaced by a newer requirement | `supersededBy` link recorded |
...
13. **MoSCoW per scope (matrix)  -  Wave 3** – Every requirement carries a **MoSCoW matrix**: rather than a single Must/Should/Could/Won't rating, each requirement is rated separately for each scope it applies to (initiative, each feature, each cohort/slice). The PM (or PO) is the source of truth for MoSCoW ratings. The BA captures and structures them.
    ...
    **Matrix structure (recorded per requirement):**
    ...
    |---|---|---|---|---|---|---|
    | REQ-001 (Business Verification check) | Must |  -  | Must | Must | Should |  -  |
