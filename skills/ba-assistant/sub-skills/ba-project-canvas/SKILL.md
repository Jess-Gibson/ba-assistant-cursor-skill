...
> **Cross-cutting rule:** This skill produces multiple artefact-class outputs (canvas .tsx, status-snapshot.html, status-data.json, optionally intake-form.canvas.tsx). Before generating outputs, apply the **"What I'll produce next" declaration** rule from `ba-assistant\SKILL.md → Co-thinking and artefact production protocol` — surface planned artefacts upfront and ask the user to select. Auto-generation triggers (`/canvas`, `/status`, end-of-Phase-0, every phase gate) are the highest-volume invocation points.
...
**Wave 3 + demo iteration (May 2026) — what's new:**
- **Workstreams tab (2nd, was "Modes")**: shows the workstream grid per scope. The 8 workstreams (no M-codes in the user-facing UI) across the top: Intake, Kickoff, Discovery, Slicing & Sequencing, Solution, Delivery, Playback, Eval & Retro. Scopes down the side (cohorts indented with `↳` and zebra-striped, divider lines between initiative/features/cohorts groups). States in cells: 🟢 Done, 🔵 Active, ⏸ Paused, ○ Not started, · N/A. **In-progress MUST be BLUE.** Compact horizontal legend BELOW the grid (not vertical chrome). The internal data model field may still be `modes` for backwards compat; section ID and all labels use "Workstreams".
- **Scope navigator (top of canvas, above tabs)**: hierarchical multi-select pill cluster with three visually-grouped sections separated by `|`: (1) "🏛️ Initiative level" pill (default selected, blue/info tone, ✓ prefix when active), (2) feature pills, (3) cohort pills. **UX contract:**
  ...
  - Selected pills get a ✓ prefix and `info` tone; unselected pills `neutral`.
...
- **Tracker tab gains Actions section**: action register is a first-class collection alongside Decisions/Risks/OQs/Assumptions. RAID cards use icons (📌🧨❓⚠️🎯). **"Show outstanding only" toggle defaults to CHECKED** so the section opens to what needs attention.
- **Timeline**: real Gantt with **Milestones swimlane at the TOP** (each milestone = emoji + label + date stacked, e.g. "⚖️ / Legal sign-off / 27 May"), then team lanes (BA Analysis, Compliance/Legal, F1 Eng, F2 Comms, etc). Vertical **blue dashed Today line** + vertical **red dashed Deadline line** with ⛔ date label. Bar length = `weeks × colWidth − 4`. Bars auto-pack into sub-rows so they never overlap. Date headers in actual dates (e.g. "18 May, 25 May…"), NOT week codes (W21, W22).
- **Emoji set**: use real Unicode throughout — 🟢 🔵 🔴 🟡 ⏸ ⛔ 📅 🚀 ⚖️ 📧 📊 🧨 ⚠️ 📌 📋 🍕 🎟️ 🏛️. Never use ASCII shapes (`✓ ● ○ —`) where Unicode emoji is available.
...
1. **`.canvas.tsx`** — the Cursor Canvas (interactive, 8 tabs)
2. **`status-snapshot.html`** — a standalone HTML file (interactive, 8 tabs, self-contained)
...
The agent MUST exhaustively read project context before generation. The "Ask the user" step is a FALLBACK for gaps — not the starting point. Read files first, ask questions only for what's genuinely missing.

### 6. PM-approval banner is MANDATORY on Phase 0 outputs (added May 2026 after [previous initiative retro])

Whenever `status-data.json → initiative.pmApproval.status` is `pending` or `requested` (the default for all Phase 0 outputs), the canvas AND the HTML snapshot MUST display a prominent banner at the top of every tab:
...
If the data model has `initiative.pmApproval` absent (legacy projects pre-May 2026), treat as `status: 'pending', pm: 'TBC'` and show the banner with `pm = 'TBC — capture in intake hook 6'`.
...
- **At Phase 0 intake completion (auto-generate, mandatory)** — initial canvas with mostly empty-state callouts. Acts as a visual roadmap of what comes next.
- At Phase 1 kickoff completion (auto-generate) — populated with stakeholders, scope, RAID
- After any phase gate (refresh)
- **After any major decision logged or RAID item added (refresh)** — keeps the canvas live as the conversation progresses, not just at gates
...
### "After major decision" refresh — when to trigger

Trigger a canvas refresh (lightweight — just overwrite both files with current
tracker state) when any of these happen mid-phase:

- A decision is recorded in the living tracker (📌)
- A risk is logged or its severity changes (🧨)
- A dependency is added or resolved (🚧)
- A sign-off is captured (✅)
- A feature slice is confirmed or deferred (Phase 3+)
- A solution option is selected (Phase 4+)

This is in addition to phase-gate refreshes. Do not refresh after every user reply —
...
E.g. `canvases/[initiative-slug]-intake-form.canvas.tsx`.
...
```json
{
  "initiative": {
    ...
    "techLead": "",
    "charterUrl": "https://...",
    "jiraProjectKey": "[YOUR-PROJECT]",
    "confluenceSpace": "[YOUR-CONFLUENCE-SPACE]",
    "allInOneDocUrl": "https://...",
    "slackChannel": "#[initiative-slug]",
    "jiraTemplate": { "sourceKey": "PROJ-003" }
  },
  "problem": "...",
  "metrics": [ "..." ],
  "scope": { "in": [ "..." ], "out": [ "..." ] },
...
export default function IntakeForm() {
  ...
  return (
    <Stack gap={20}>
      <H1>Intake Form — [Example Initiative]</H1>
...
Threshold for warning: below 70% for any scope with active Delivery. Trend matters more than absolute value — a falling rate is the signal.
...
Threshold for warning: below 95% for any scope past Discovery completion. Below 95% means at least one requirement entered the register without challenge — which is the failure mode the Interrogator exists to prevent.
...
- `<workspace>/blueprints/<project-name>/docs/blueprints/analysis/` — primary analysis folder
- `<workspace>/blueprints/Project NNN - <name>/docs/blueprints/analysis/` — numbered project format
...
```
getAccessibleAtlassianResources → pick the cloud ID for [your-instance].atlassian.net
```
If your Atlassian instance is not in the list, Jira auth is missing for this session — fall back to markdown sources and note the gap in the canvas footer.
...
searchJiraIssuesUsingJql:
  jql: "project = [YOUR-PROJECT] AND issueType = Epic AND 'Epic Link' = <programme-key> ORDER BY key ASC"
  OR: "project = [YOUR-PROJECT] AND parent = <programme-key> ORDER BY key ASC"
  fields: [summary, status, assignee, labels, priority, customfield_10016, customfield_10004]
  maxResults: 50
```

**3d. Fetch all stories under each epic**
For each epic found (or directly if user provides an epic key like PROJ-004):
```
searchJiraIssuesUsingJql:
...
- Add the actual date to the label: `"PROJ-005 Domain Model (15 May)"` — this makes the timeline self-documenting
...
1. **Extract sprint dates from the API response.** The sprint field (`customfield_10007`) returns an array with `startDate` and `endDate` as ISO timestamps (e.g. `"2026-05-19T13:00:00.000Z"`). The sprint `name` also often encodes the date in a team-specific convention (e.g. `[YOUR-PROJECT] Sprint 37 20260513-527` → `20260513` = 13 May).
...
```
Example: PROJ-002 is in Sprint 37 (starts 13 May), project kickoff was 28 Apr.
  sprintStartDate = 13 May 2026
  startWeek = (13 May - 28 Apr) / 7 = ~2.1
  weeks = 2 (default sprint length)
  label = "PROJ-002 Churned State CS (Sprint 37, est.)"
  status = "pending"
```
...
1. For each ticket, record `storyPoints = issue.fields.customfield_10016 ?? issue.fields.customfield_10004` (use whichever is non-null; `customfield_10004` is "Story Points (classic)" common in some Jira instances).
...
|---|---|
| `label: "F-A Rule Uplift"` | `label: "Compliance Rule Uplift"` |
| `shortLabel: "Feature A"` | `shortLabel: "Rule Uplift"` |
...
7. **Hover tooltips** — SVG `<title>` elements inside each `<g>` node. Format examples:
   - Workstream cell: `"F1 Feature Detection — Solution: Done"`
   - Gantt bar: `"F1 Solo deploy — In progress — scope: Solo"`
   - Milestone: `"Legal sign-off — 27 May"`
   - Dep / trace node: `"PROJ-006: block regulatory deadline ≥ 1 Jul (in progress)"`
...
8. **Include actual dates in timeline labels.** For done and in-progress tickets, append the start date to the label: `"PROJ-005 Domain Model (15 May)"`. This makes the timeline self-documenting and verifiable at a glance without needing to cross-reference Jira.
...
**Filter:** `filteredNodes = depNodes.filter((n) => isActive(n.scope) || n.id === "deadline")` — always keep the final-deadline node visible regardless of scope so the graph terminates somewhere meaningful.
...
- ⚖️ `driver` (e.g. [Regulatory Driver]) — `theme.diff.stripRemoved`
...
7. **Decisions table MUST include Made by + Status** — this matches `ba-profile.mdc`'s "Decisions must be recorded as a table" requirement.
...
| **Week 0 = project start** | Does the timeline start from the D1 kickoff week, not "today"? For active projects, "today" should be ~week 3–8, not week 0. |
...
For example: `blueprints/[Project-Folder]/docs/blueprints/analysis/status-snapshot.html`
...
- Use a **clean, professional layout** with:
  ...
  - Timeline Gantt rendered as inline SVG (matches the [initiative-slug] canvas pattern — swimlanes as rect rows, bars as positioned rects, today/deadline as vertical dashed lines, milestone strip as the TOP lane)
...
The canonical reference for both outputs is your **local demo or reference canvas** (if available):
- Canvas: `~/.cursor/projects/<workspace-slug>/canvases/[initiative-slug]-demo.canvas.tsx`
- HTML: `~/.cursor/projects/<workspace-slug>/canvases/[initiative-slug]-demo.html`
