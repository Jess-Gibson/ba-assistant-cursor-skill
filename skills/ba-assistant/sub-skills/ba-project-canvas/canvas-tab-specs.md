...
**Filter:** `filteredNodes = depNodes.filter((n) => isActive(n.scope) || n.id === "Sample Initiative")`  -  always keep the final-deadline node visible regardless of scope so the graph terminates somewhere meaningful.
...
- ⚖️ `driver` (e.g. Sample Initiative Mandate)  -  `theme.diff.stripRemoved`
...
- Use a **clean, professional layout** with:
  ...
  - Timeline Gantt rendered as inline SVG (matches the sample-data-collection canvas pattern  -  swimlanes as rect rows, bars as positioned rects, today/deadline as vertical dashed lines, milestone strip as the TOP lane)
