...
| Hook | When | Why |
|---|---|---|
| **ba-data-investigation** (`HK-EVAL-BDI-actual`) | Always | Pull actual metrics (warehouse, logs, Segment, FullStory, support ticket volume) for the success metrics defined at intake, cross-validated against a second source where one exists. No evaluation without evidence  -  this hook is a block, not a warn. |
...
2. **Pull actual data (`HK-EVAL-BDI-actual`)**  -  Hand off to `ba-data-investigation` with:
   ...
   - The data sources (warehouse tables, logs queries, Segment events, support ticket categories)
...
9. **Decisions log**  -  what was decided, by whom, by when (table format per [BA name] BA profile)
