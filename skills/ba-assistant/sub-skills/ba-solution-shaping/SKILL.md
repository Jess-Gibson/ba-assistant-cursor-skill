...
7. **Downstream path analysis (integration contract matrix)** – For initiatives that span multiple services or systems, trace each business path through all downstream integration points. Produce an **integration contract matrix**:
   ...
   |---|---|---|---|---|---|
   | e.g. Reassessment → APPROVED | Reassessment completes | Risk service | Approval event | Segment function | Close support tool ticket |
   | e.g. Manual approval | Ops approves in support tool | AMS | Approval event | Segment function | Close support tool ticket |
   ...
   **Origin:** Retro 29 Jun 2026 (sample-reassessment-initiative)  -  three go-live blockers (PROJ-4437, PROJ-4438, PROJ-4439) were knowable from the requirements but weren't caught until bug bash because downstream path coverage was never systematically traced.
