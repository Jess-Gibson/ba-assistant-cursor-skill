...
8. **Track sponsor decisions explicitly**  -  Every sponsor decision gets a row in the decisions table (per [BA name] BA profile): `ID | Decision | Made by | Date | Status`. If the sponsor verbally agrees but it's never written down, it's not a decision  -  it's a hope.
