...
- Auto-detect the template from context (e.g. Phase 2 discovery → Template 3)
...
- **Use the verified template patterns** from `miro-board-analysis`:
  - Full workshop → **Pattern 1** (single wide horizontal frame, sections in agenda order)
  - Brainstorming → **Pattern 2** (prompt sticky + seed examples + open space)
  - Q&A grids → **Pattern 2b** (dark_blue questions, light_yellow responses)
  - RAID capture → **Pattern 7b** (four bordered columns with coloured stickies)
  - Decisions/outcomes → **Pattern 4** (table with header + grey backdrop)
  - Dot voting → **Pattern 6** (options as shapes + empty vote areas)
