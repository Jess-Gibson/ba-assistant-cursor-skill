...
**⚠️ `STICKY` has a fixed aspect ratio  -  provide EITHER `w` OR `h`, never both, and never rely on it for controlled-height cards.** At narrow widths (like the `w=365` example below) this is invisible. At the widths needed for readable multi-line question text (`w=800+`), the proportionally-scaled height becomes far taller than any reasonable vertical spacing, guaranteeing overlap. This broke the Sample Initiative [Team Member]/[Team Member] board's Q&A pairs at `w=850` (Jul 2026)  -  see `recurring-mistakes.md`.

**Use this pattern only for short, single-line prompts at narrow widths (≤400px).** For anything wider or needing predictable height, use the SHAPE-based Card Pair pattern below instead.
...
### Q&A / Decision Card Pair (SHAPE-based  -  use for wide or multi-line content)

Use explicit `SHAPE` rectangles instead of `STICKY` whenever the card needs both a specific width AND a specific height (e.g. any question/decision card wide enough for readable body text).

```
q1 SHAPE parent={frameUrl} x=2200 y=307 w=850 h=180 type=rectangle fill=#232428 fill_opacity=1.0 color=#ffffff size=30 align=left valign=top border_color=#ffffff border_style=normal border_width=1.0 border_opacity=1.0 "Your question text here?"
a1 SHAPE parent={frameUrl} x=2200 y=507 w=850 h=160 type=rectangle fill=#ffffff fill_opacity=1.0 color=#1a1a1a size=30 align=left valign=top border_color=#7b14ef border_style=dashed border_width=2.0 border_opacity=1.0 ""
```
...
```
TEXT parent={frameUrl} x=1104 y=1384 w=122 color=#7b14ef fill=#ffffff fill_opacity=0.0 font=unknown size=39 align=left "<p><u style=\"color:rgb(123,20,239)\">[Organisation]</u></p>"
```
